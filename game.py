...
while 1:
	...
	if copyMouseY>0:
		if counterTimer%2==0:
			screen.blit(shot,(mouseX,copyMouseY))
			shot_rect=shot.get_rect()
			shot_rect.move_ip(mouseX,copyMouseY)
			...
			if shotColNumber==2:
				...
				#a=points      #Storing initial value
				pointAdder=score(top)
				#pointAdderSTR=str(pointAdder)
				#listReturnP=myText((pointAdderSTR),25)
				#Ptext=listReturnP[0]
				#screen.blit(Ptext,(mouseX+5,copyMouseY+5))    #Displaying score adder close to where it hits
				points+=pointAdder
				'''
				b=points		#Storing final value
				if a<100 and b>=100:
					pointctr+=1
					if a%(pointctr*100)<(pointctr*100) and b%(pointctr*100)>=(pointctr*100):
						speed()
				'''
				copyMouseY=1
				counterTimer=0
	...
	listReturn6=myText("Lives Remaining: ",25)
	livesText=listReturn6[0]
	screen.blit(livesText,(420,980))

	turnsSTR=str(5-turns)

	listReturn7=myText(turnsSTR,25)
	livesText=listReturn7[0]
	screen.blit(livesText,(670,980))
